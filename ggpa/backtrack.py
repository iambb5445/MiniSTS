...
class BacktrackBot(GGPA):
    ...
    def _get_best_choose_card(self, game_state: GameState, battle_state: BattleState, depth_remaining: int) -> tuple[int|None, PlayCard|EndAgentTurn|None]:
        if depth_remaining == 0:
            x = self._evaluate_state(game_state, battle_state)
            return x, None
        ...
        for option in options:
            ...
            battle_state_copy.tick_player(option)
            estimate = self._get_best_choose_card(battle_state_copy.game_state, battle_state_copy, depth_remaining-1)
            if best_value is None or (estimate[0] is not None and best_value < estimate[0]):
                best_value = estimate[0]
                best_action = option
    ...
    def choose_card(self, game_state: GameState, battle_state: BattleState) -> EndAgentTurn|PlayCard:
        ...
        if action is None:
            raise Exception("Depth is 0 or no action available")
        return action
